...
public class Transaction {
    ...
    public Transaction(UUID id, User recipient, User sender, Transaction.Category transferCategory, double transferAmount) throws Exception {
        setId(id);
    ...
    public void setTransferAmount(double transferAmount){
        try{
            if (transferCategory == Transaction.Category.DEBIT && transferAmount >= 0 || transferCategory == Transaction.Category.CREDIT && transferAmount < 0)
    ...
    private void transactionInit(User recipient, User sender,Transaction.Category transferCategory, double transferAmount) throws Exception {

            if (sender.getBalance() >= transferAmount && transferCategory == Category.CREDIT) {
                sender.setBalance(sender.getBalance() + transferAmount);
             //   recipient.setBalance(recipient.getBalance() + transferAmount);
            } else if (transferCategory == Category.DEBIT){
                recipient.setBalance(recipient.getBalance() + transferAmount);
            } else {
